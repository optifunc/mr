...
for (const readonly of [false, true]) {
    test(`reviewed Up/Down examples with actual keys (${readonly ? 'read-only' : 'editable'})`, async ({ page, browser }, info) => {
        ...
        if (!readonly) {
            writeFileSync(`docs/evidence/milestone-d/regression/navigation-root/results-${info.project.name}.json`, JSON.stringify({ browser: browser.version(), results }, null, 2) + '\n');
            await page.locator('#primary').screenshot({ path: `docs/evidence/milestone-d/regression/navigation-root/selection-${info.project.name}.png` });
        }
    });
}
...
for (const readonly of [false, true]) {
    test(`root Up/Down is a no-op with plain and Shift keys (${readonly ? 'read-only' : 'editable'})`, async ({ page }, info) => {
        ...
        for (const ids of [['root'], ['one', 'root']]) {
            ...
            if (!readonly && ids.length === 1) await page.locator('#primary').screenshot({ path: `docs/evidence/milestone-d/regression/navigation-root/root-${info.project.name}.png` });
        }
    });
}
