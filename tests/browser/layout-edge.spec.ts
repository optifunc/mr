...
test('explicit newlines retain every empty row in measurement, rendering, and selection', async ({ page }, info) => {
    ...
    writeFileSync(`docs/evidence/milestone-c/regression/newlines-${info.project.name}.json`, JSON.stringify(results, null, 2) + '\n');
    await page.locator('#secondary').scrollIntoViewIfNeeded();
    await page.evaluate(() => new Promise<void>(resolve => requestAnimationFrame(() => requestAnimationFrame(() => resolve()))));
    await page.locator('#secondary').screenshot({ path: `docs/evidence/milestone-c/regression/newlines-${info.project.name}.png` });
});
...
test('ancestor transforms do not enter local measurements at mount or refresh', async ({ page }, info) => {
    ...
    writeFileSync(`docs/evidence/milestone-c/regression/scaling-${info.project.name}.json`, JSON.stringify(results, null, 2) + '\n');
    await page.locator('#secondary').scrollIntoViewIfNeeded();
    await page.evaluate(() => new Promise<void>(resolve => requestAnimationFrame(() => requestAnimationFrame(() => resolve()))));
    await page.locator('#secondary').screenshot({ path: `docs/evidence/milestone-c/regression/scaling-${info.project.name}.png` });
});
