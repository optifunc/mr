...
for (const zoom of [1, 1.5, 2]) test(`F2 width follows visible children and text/bottom alignment at ${zoom}`, async ({ page }, info) => {
    ...
    for (const id of ['one', 'two', 'child2', 'a', 'child1', 'collapsed', 'chain']) {
        ...
        const actual = await editor.evaluate(e => {
            ...
            return { box: r.toJSON(), paddingLeft: parseFloat(s.paddingLeft), paddingTop: parseFloat(s.paddingTop), scrollLeft: e.scrollLeft, scrollTop: e.scrollTop };
        });
        ...
        expect.soft(Math.abs(actual.box.width - (before.expanded ? before.node.width : (Math.ceil(before.eightM) + 6) * zoom))).toBeLessThan(.1);
        ...
        if (before.expanded || id !== 'chain') expect.soft(Math.abs(actual.box.x + (1 + actual.paddingLeft - actual.scrollLeft) * zoom - before.label.x)).toBeLessThan(.8);
