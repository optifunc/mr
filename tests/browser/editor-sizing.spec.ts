...
for (const zoom of [1, 1.5, 2]) test(`F2 width follows visible children and text/bottom alignment at ${zoom}`, async ({ page }, info) => {
    ...
    for (const id of ['one', 'two', 'child2', 'a', 'child1', 'collapsed', 'chain']) {
        ...
        const actual = await editor.evaluate(e => {
            ...
            return { box: r.toJSON(), label: label.toJSON(), borderLeft: parseFloat(s.borderLeftWidth), borderTop: parseFloat(s.borderTopWidth), paddingLeft: parseFloat(s.paddingLeft), paddingTop: parseFloat(s.paddingTop), scrollLeft: e.scrollLeft, scrollTop: e.scrollTop };
        });
        ...
        expect.soft(Math.abs(actual.box.y + (actual.borderTop + actual.paddingTop - actual.scrollTop) * zoom - before.label.y)).toBeLessThan(.1);
        ...
        expect.soft(Math.abs(actual.box.x + (actual.borderLeft + actual.paddingLeft - actual.scrollLeft) * zoom - actual.label.x)).toBeLessThan(.8);
