...
test('workload early full-relayout diagnostic', async ({ page, browser }, info) => {
    ...
    writeFileSync(`docs/evidence/milestone-a/workload-${info.project.name}.json`, JSON.stringify(report, null, 2) + '\n');
    await page.locator('#primary').screenshot({ path: `docs/evidence/milestone-a/workload-${info.project.name}.png` });
});
