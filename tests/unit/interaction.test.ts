...
test('vertical navigation stays at the same depth across groups; root vertical arrows do nothing', () => {
    const s = new Store({ document: referenceMap() }), g = geometry(s);
    ...
    expect(navigate(s.model, g, 'root', 'up')).toEqual({});
    expect(navigate(s.model, g, 'root', 'down')).toEqual({});
