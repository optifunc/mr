...
export function prepare(model: Model, selection: Selection, command: MindMapCommand, createId: () => string, order?: readonly string[]): Prepared | undefined {
    ...
    const get = (id: string | undefined): NodeRecord => { const n = id === undefined ? undefined : nodes.get(id); if (!n)
        throw new MindMapError('INVALID_TARGET', `Unknown node: ${id ?? '(none)'}`); return n; };
    ...
    if ('ids' in command && command.ids !== undefined)
        command.ids.forEach(get);
    if (explicit !== undefined)
        get(explicit);
    const choose = (id: string): void => { nextSelection = { ids: [id], activeId: id }; };
    const newId = (): string => { let id: string; try {
        id = createId();
    }
    catch {
        throw new MindMapError('HOST_CALLBACK', 'createNodeId threw');
    } if (typeof id !== 'string' || !id || nodes.has(id))
        throw new MindMapError('INVALID_ID', 'createNodeId must return a new nonempty ID'); return id; };
    ...
    switch (command.type) {
        ...
        case 'insertParent': {
            ...
            const atRoot = target.id === model.rootId;
            const wrap = command.type === 'insertParent' && !atRoot;
            let parent = command.type === 'insertChild' || atRoot ? target.id : target.parent!;
            let index = get(parent).children.length;
            let side: RootSide | undefined;
            if (parent === model.rootId) {
                side = atRoot ? command.type === 'insertParent' ? 'left' : 'right' : target.side!;
                index = atRoot ? rootInsertIndex(side, command.type === 'insertBefore') : get(parent).children.indexOf(target.id) + (command.type === 'insertAfter' ? 1 : 0);
            }
            ...
            if (wrap) {
                ...
                nodes.set(target.id, { ...rest, parent: id });
            }
            if (get(parent).collapsed)
                update(parent, { collapsed: false });
            choose(id);
            break;
        }
        case 'setText':
            if (!targetId)
                return;
            update(targetId, { text: command.text });
            break;
        case 'toggleCollapse':
        case 'collapse':
        case 'expand': {
            ...
            if (collapsed) {
                ...
                const visibleSelection = normalizeSelection(candidate, nextSelection);
                if (visibleSelection.ids.length !== nextSelection.ids.length)
                    nextSelection = { ids: [...new Set([...visibleSelection.ids, target.id])], activeId: target.id };
            }
            break;
        }
        case 'toggleChecked':
        ...
        case 'removeCheckbox': {
            ids.forEach(get);
            const targets = ids.map(get);
            const checked = targets.filter(n => n.checked !== undefined);
            const value = checked.some(n => !n.checked);
            for (const n of targets) {
                if (command.type === 'toggleChecked' && n.checked !== undefined)
                    update(n.id, { checked: value });
                if (command.type === 'addCheckbox' && n.checked === undefined)
                    update(n.id, { checked: false });
                if (command.type === 'removeCheckbox' && n.checked !== undefined) {
                    const { checked: _checked, ...rest } = n;
                    nodes.set(n.id, rest);
                }
            }
            break;
        ...
        case 'delete': {
            ids.forEach(get);
            if (!ids.length || ids.includes(model.rootId))
                return;
            const roots = normalizeRoots(model, ids, order);
            const removed = new Set<string>();
            const stack = [...roots];
            while (stack.length) {
                const n = get(stack.pop()!);
                removed.add(n.id);
        ...
        case 'move': {
            ...
            const target = get(command.destination.targetId);
            if (moving.has(target.id))
                throw new MindMapError('INVALID_TARGET', 'Cannot move onto the moving group');
            let ancestor: string | null = target.id;
            while (ancestor) {
                if (moving.has(ancestor))
                    throw new MindMapError('INVALID_TARGET', 'Cannot move into a descendant');
                ancestor = get(ancestor).parent;
            }
            const position = command.destination.position;
            if (target.id === model.rootId && position !== 'child')
                throw new MindMapError('INVALID_TARGET', 'Cannot insert beside root');
            ...
            for (const id of roots) {
                ...
                nodes.set(id, { ...rest, parent, ...(side ? { side } : {}) });
            }
            ...
            if (visibleSelection.ids.length !== nextSelection.ids.length) {
                let visibleTarget = target.id;
                const visible = new Set(normalizeSelection({ rootId: model.rootId, nodes }, { ids: [...nodes.keys()] }).ids);
                while (!visible.has(visibleTarget))
                    visibleTarget = get(visibleTarget).parent!;
                choose(visibleTarget);
            }
            break;
        }
        default: return;
