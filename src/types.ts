...
export interface RootChild extends MindMapNode {
    side: RootSide;
}
...
export type MindMapCommand = {
    type: 'insertChild' | 'insertBefore' | 'insertAfter' | 'insertParent';
    targetId?: NodeId;
    text?: string;
} | {
    type: 'setText';
    ...
    type: 'delete' | 'toggleChecked' | 'addCheckbox' | 'removeCheckbox' | 'copy' | 'cut';
    ids?: NodeId[];
} | {
    type: 'toggleCollapse' | 'expand' | 'collapse' | 'edit' | 'paste' | 'openLink';
    targetId?: NodeId;
} | {
    type: 'move';
    ids?: NodeId[];
...
export class MindMapError extends Error {
    constructor(public readonly code: ErrorCode, message: string) { super(message); this.name = 'MindMapError'; }
}
