...
export function validateDocument(input: unknown): Model {
    if (!input || typeof input !== 'object' || !('root' in input))
        return invalid('Expected a document with one root');
    ...
    const work: {
        ...
        parent: string | null;
        rootChild: boolean;
    }[] = [{ value: input.root, parent: null, rootChild: false }];
    let rootId = '';
    while (work.length) {
        const { value, parent, rootChild } = work.pop()!;
        if (!value || typeof value !== 'object' || Array.isArray(value))
            return invalid('Expected a node object');
        if (seen.has(value))
            return invalid('Cycles and shared node objects are not trees');
        seen.add(value);
        const n = value as Record<string, unknown>;
        if (typeof n.id !== 'string' || !n.id.length || nodes.has(n.id))
            return invalid('IDs must be unique nonempty strings');
        if (typeof n.text !== 'string' || !Array.isArray(n.children))
            return invalid('Nodes need string text and a children array');
        for (const key of ['checked', 'collapsed'])
            if (key in n && typeof n[key] !== 'boolean')
                return invalid(`${key} must be boolean when present`);
        if (rootChild && n.side !== 'left' && n.side !== 'right')
            return invalid('Root children need a left or right side');
        ...
        for (let i = n.children.length - 1; i >= 0; i--)
            work.push({ value: n.children[i], parent: record.id, rootChild: parent === null });
    }
    return { rootId, nodes };
}
export function snapshot(model: Model): MindMapDocument {
    ...
    for (const n of model.nodes.values())
        copies.set(n.id, { id: n.id, text: n.text, children: [], ...(n.side ? { side: n.side } : {}), ...(n.checked !== undefined ? { checked: n.checked } : {}), ...(n.collapsed !== undefined ? { collapsed: n.collapsed } : {}) });
    for (const n of model.nodes.values())
        copies.get(n.id)!.children = n.children.map(id => copies.get(id)!);
...
export function normalizeRoots(model: Model, ids: readonly string[], order: readonly string[] = visibleIds(model)): string[] {
    ...
    return [...selected].filter(id => { let p = model.nodes.get(id)?.parent; while (p) {
        if (selected.has(p))
            return false;
        p = model.nodes.get(p)?.parent;
    } return model.nodes.has(id); }).sort((a, b) => (rank.get(a) ?? Infinity) - (rank.get(b) ?? Infinity));
}
export function normalizeSelection(model: Model, selection: Selection): Selection {
    const visible = new Set(visibleIds(model));
...
export function generateId(): string {
    ...
    if (!globalThis.crypto?.getRandomValues)
        throw new MindMapError('INVALID_ID', 'Secure ID generation is unavailable; supply createNodeId');
