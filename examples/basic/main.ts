...
<header><h1>Mind map <span>Milestone D · complete</span></h1>
<p>Milestone D is complete and accepted: editing, clipboard, dragging, menus and host integration. Release checks and known gaps are linked below.</p></header>
<main>
...
<section><h2>Menus and host integration</h2><p>Right-click a node or focus the map and press Shift+F10 / Context Menu. Use arrows, Home/End and Enter; Escape/Tab returns to the canvas. Disabled items stay visible. A selected group is preserved on right-click. Try checkbox actions, insertions, copy/paste, undo, and the URL fixture.</p><div class="demo-actions"><button id="snapshot-action">Inspect document snapshot</button><button id="invalid-replacement">Try invalid replacement</button><a href="/?readonly">Read-only demo</a><a href="/?no-menu">Host menu disabled</a><a href="/examples/consumer/">Built-package integration and mount/destroy demo</a><a href="/examples/performance/">Isolated 1,000 / 500 workload</a></div><details><summary>Current document snapshot</summary><pre id="snapshot"></pre></details><p>The independent mount above demonstrates checkbox, multiline and plain-text labels. The integration example includes read-only mode, repeated mounting, cleanup and preserved host content.</p></section>
<section><h2>Stage-9 review evidence</h2><p><a href="/docs/evidence/milestone-d/checkbox-shortcut/report.md">Checkbox shortcut and menu hints</a> · <a href="/docs/evidence/milestone-d/delete-selection/report.md">Deletion selection update and checks</a> · <a href="/docs/evidence/milestone-d/review-fixes/report.md">P2 review fixes and refreshed verification</a> · <a href="/docs/evidence/milestone-d/menu-focus/report.md">Context-menu appearance update: before/after and checks</a> · <a href="/docs/evidence/milestone-d/report.md">Milestone D report, test results, screenshots and known gaps</a> · <a href="/docs/evidence/milestone-d/browser.txt">Full browser results</a> · <a href="/docs/evidence/milestone-d/review-fixes/performance/report.md">Performance measurements</a> · <a href="/docs/evidence/milestone-d/review-fixes/package/result.json">Isolated packaged consumer results</a> · <a href="/docs/acceptance.md">All acceptance criteria and product exercises</a></p><p>Milestone D was accepted on 2026-09-10 at 497eff9. Safari automation, current stable browser coverage, manual VoiceOver/NVDA and physical input-to-display verification retain their recorded release gaps. Automated tests do not waive these checks.</p></section>
</main>`;
const primary = new MindMapEditor(document.querySelector('#primary')!, { document: workload ? workloadMap() : referenceMap(), readonly, contextMenu: !params.has('no-menu') });
...
act('snapshot-action', () => { document.querySelector('#snapshot')!.textContent = JSON.stringify(primary.getDocument(), null, 2); document.querySelector('#snapshot')!.closest('details')!.open = true; });
act('invalid-replacement', () => primary.setDocument({ root: { id: '', text: 'Invalid ID', children: [] } }));
