...
<section><h2>Clipboard and restructuring exercises</h2><div class="demo-actions"><button id="clipboard-fixture">Load clipboard + links fixture</button><button id="copy">Copy selection</button><button id="cut">Cut selection</button><button id="paste">Paste into active node</button><button id="link">Open active link</button><button id="drag-reset">Reset drag comparison</button></div><p>Command/Ctrl+C, X and V use the system clipboard. Copy mixed checkbox/multiline branches from the fixture and paste into another node. Undo and redo each operation once. Command/Ctrl+click opens a URL label; its branch line still toggles selection. The event panel above reports clipboard completion or errors.</p><p>Select B and C, then drag either label. The darkest edge shows the result: top inserts before, bottom after, and the outward edge adds children. Root halves choose the side. Top/bottom quarters win at corners; the inward middle and ineffective moves show a prohibited cursor with no gradient. Escape cancels; the viewport pans near an edge. Drop into Collapsed node and expand its circle to find the moved children.</p></section>
