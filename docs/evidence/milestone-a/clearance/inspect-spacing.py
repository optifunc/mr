...
for browser in ("chromium", "firefox", "webkit"):
    sources[f"before-{browser}"] = evidence.parent / f"spacing/reference-{browser}.png"
    sources[f"after-{browser}"] = evidence / f"reference-{browser}.png"


def ink_rows(image, x1, y1, x2, y2, cutoff=85):
    # Crop above the branch; report faint descender edges separately from dark ink.
    return [y for y in range(y1, y2)
            if any(min(image.getpixel((x, y))) < cutoff for x in range(x1, x2))]
...
for name, path in sources.items():
    ...
    results[name] = {
        ...
        "descender_blank_rows_dark": single_line - max(ink_rows(im, 335, 125, 420, single_line)) - 1,
        "descender_blank_rows_including_faint_edges": single_line - max(ink_rows(im, 335, 125, 420, single_line, 255)) - 1,
...
print(output)
for browser in ("chromium", "firefox", "webkit"):
    before, after = results[f"before-{browser}"], results[f"after-{browser}"]
    assert after["descender_blank_rows_including_faint_edges"] >= 2, browser
    assert after["descender_blank_rows_including_faint_edges"] > before["descender_blank_rows_including_faint_edges"], browser
    assert after["single_line_blank_rows_to_n1"] == results["reference"]["single_line_blank_rows_to_n1"], browser
print("PASS: visible descender clearance and gap above N1 in all three engines")
